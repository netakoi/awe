...
class RobomimicImageRunner(BaseImageRunner):
    ...
    def __init__(
        ...
        import gym
        from gym.spaces import Tuple as GymTuple, Dict as GymDict, Box
        def dummy_env_fn():
            ...
            # 🔧 PATCH: wrap raw observation_space if needed
            obs_space = env.observation_space
            if isinstance(obs_space, tuple):
                print(">>> [PATCH] Wrapping raw tuple as gym.spaces.Tuple")
                env.observation_space = GymTuple(list(obs_space))
            elif isinstance(obs_space, dict) and not isinstance(obs_space, GymDict):
                print(">>> [PATCH] Wrapping raw dict as gym.spaces.Dict")
                env.observation_space = GymDict(obs_space)
        ...
        from gym.spaces import Tuple as GymTuple, Dict as GymDict

        def make_valid_env():
            env = dummy_env_fn()
            if isinstance(env.observation_space, tuple):
                from gym.spaces import Tuple as GymTuple
                print(">>> [FIX] Wrapping observation_space tuple into gym.spaces.Tuple")
                env.observation_space = GymTuple(list(env.observation_space))
            return env

        env_fns = [make_valid_env for _ in range(1)]

        # Use the same patched env for dummy_env_fn
        env = AsyncVectorEnv(env_fns, dummy_env_fn=make_valid_env, shared_memory=False)
    ...
    def run(self, policy: BaseImagePolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            try:
                ...
                print("FATAL ENV RESET FAILURE:", e)
                raise
